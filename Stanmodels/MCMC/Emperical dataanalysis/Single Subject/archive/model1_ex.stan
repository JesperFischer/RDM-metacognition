...
transformed parameters{
  ...
  for(i in 1:N){
    ...
    if(a[i] == 1){
      mu_conf[i] = inv_logit(2*ehat[i]*abs(ehat[i]) / (exp(sigma_e_log)^2 + exp(sigma_m_log)^2));
    }else if(a[i] == 0){
      mu_conf[i] = 1- inv_logit(2*ehat[i]*abs(ehat[i]) / (exp(sigma_e_log)^2 + exp(sigma_m_log)^2));
    }
  }
}
...
model {
  ...
  prec_conf_log ~ normal(5, 3);
  // prec_conf_log ~ normal(-5, 5);
  ...
  // target += normal_lpdf(C | (mu_conf), exp(prec_conf_log));
...
generated quantities{
  ...
  for(i in 1:N){
    ...
    loglik[i] = loglik_act[i] + loglik_conf[i];
