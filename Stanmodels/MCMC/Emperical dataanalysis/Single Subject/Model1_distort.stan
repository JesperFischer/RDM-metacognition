functions{
  real p_distort(real p, real gamma, real p0) {
    return(inv_logit(gamma * logit(p) + (1 - gamma) * logit(p0)));
  }
...
model {
  ...
  for(i in 1:N){
    ...
    target += ord_beta_reg_lpdf(C[i] | logit(p_distort(mu_conf[i], exp(gamma),inv_logit(p0))), exp(prec_conf_log), c0, c11);
