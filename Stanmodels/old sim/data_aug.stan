...
transformed parameters{
  ...
  for(i in 1:N){
    ...
    real like_e_correct = exp(normal_lpdf(evidence[i] | exp(beta) * (a[i] * X[i] - alpha), 1))*(0.5+inv_logit(delta)/2);

    real like_e_incorrect = exp(normal_lpdf(evidence[i] | exp(beta) * (-a[i] * X[i] - alpha), 1))*(0.5-inv_logit(delta)/2);

    confidence[i] = like_e_correct / (like_e_correct + like_e_incorrect);
  }
}
