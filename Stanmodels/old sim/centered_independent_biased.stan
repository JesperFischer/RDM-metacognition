...
parameters {
  ...
  vector[N] evidence;      // CENTERED: directly sample evidence
  vector[N] evidence_m;    // CENTERED: directly sample metacognitive evidence
  real c0;
  real c11;
}
...
transformed parameters{
  vector[N] confidence;
  real p_correct = 0.5+inv_logit(delta)/2;
  ...
  for(i in 1:N){
    ...
    real loglike_e_correct = (normal_lpdf(evidence[i] | exp(beta) * (a[i] * X[i] - alpha), 1));

    real loglike_e_incorrect = (normal_lpdf(evidence[i] | exp(beta) * (-a[i] * X[i] - alpha), 1));
    ...
    real logp_cor = loglike_e_correct + loglike_m_correct + log(p_correct);
    
    real logp_incor = loglike_e_incorrect + loglike_m_incorrect + log1m(p_correct);
...
model {
  ...
  beta_diff ~ normal(0, 0.5);    // Prior on difference (often small)
  delta  ~ normal(-2, 0.5);
  conf_prec ~ normal(0,3);
  
  // CENTERED: Add priors directly on evidence
  for (i in 1:N) {
    evidence[i] ~ normal(exp(beta) * (D[i] * X[i] - alpha), 1);
    evidence_m[i] ~ normal(exp(beta_m) * (D[i] * X[i] - alpha), 1);
  }
...
generated quantities{
  real beta1 = exp(beta_mean + beta_diff / 2.0);
  real beta_m1 = exp(beta_mean - beta_diff / 2.0);
}
