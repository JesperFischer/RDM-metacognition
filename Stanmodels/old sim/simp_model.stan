...
model {
  ...
  gm[6] ~ normal(0,2); //global mean of meta bias
  ...
  for (n in 1:N) {
    ...
    u_mix[n, 2] = ord_beta_reg_cdf(Conf[n] | logit(conf_mu[n]) + meta_bias, exp(conf_prec), c0, c11);
...
generated quantities {
 ...
 matrix[N, 2] u_mixx;
  for (n in 1:N) {
    ...
    u_mixx[n, 2] = ord_beta_reg_cdf(Conf[n] | logit(conf_mu[n])+ meta_bias, exp(conf_prec), c0, c11);
  }
  ...
  rho_p_conf = multiply_lower_tri_self_transpose(rho_chol)[1, 2];
  ...
  for(n in 1:N){
    ...
    log_lik[n] = log_lik_bin[n] + log_lik_conf[n] + log_lik_cop[n];
  }
