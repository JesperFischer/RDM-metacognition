...
parameters {
  ...
  real conf_prec;               // sensitivity (on evidence scale)
  real delta;               // sensitivity (on evidence scale)
...
transformed parameters{
  ...
  for(i in 1:N){
    ...
    // e_i ~ N(exp(beta) * (D[i] * X[i] - alpha), 1)
    ...
    real logp_cor = loglike_e_correct + log(p_correct);
    
    real logp_incor = loglike_e_incorrect  + log1m(p_correct);
