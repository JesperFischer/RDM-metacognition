...
transformed parameters{
  ...
  real meta_bias_int = gm[10];
  real meta_bias_beta = gm[11];
...
model {
  ...
  for (n in 1:N) {
    ...
    u_mix[n, 3] = ord_beta_reg_cdf(Conf[n] | logit(conf_mu[n]) + meta_bias_int + interval[n] * meta_bias_beta, exp(conf_prec), c0, c11);

    target += lognormal_lpdf(RT[n] - rt_ndt | rt_int + rt_slope * entropy_t[n], exp(rt_prec));

    target += ord_beta_reg_lpdf(Conf[n] | logit(conf_mu[n]) + meta_bias_int + interval[n] * meta_bias_beta, exp(conf_prec), c0, c11);
...
generated quantities {
 ...
 matrix[N, 3] u_mixx;
  for (n in 1:N) {
    ...
    u_mixx[n, 3] = ord_beta_reg_cdf(Conf[n] | logit(conf_mu[n]) + meta_bias_int + interval[n] * meta_bias_beta, exp(conf_prec), c0, c11);
  }
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(Conf[n] | logit(conf_mu[n]) + meta_bias_int + interval[n] * meta_bias_beta, exp(conf_prec), c0, c11);
