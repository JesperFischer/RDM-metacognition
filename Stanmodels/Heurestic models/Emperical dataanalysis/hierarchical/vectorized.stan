...
transformed parameters{
  ...
  for(n in 1:N){
      sigma1_sq_n[n] = sqrt(square(exp(sigma_e[S_id[n]])) + square(exp(sigma_k[S_id[n]])));
      sigma2_n[n] = square(exp(sigma_e[S_id[n]])) + square(exp(sigma_m[S_id[n]]  +  sigmam_beta[S_id[n]] * interval[n]));
      sigma_e_n[n] = square(exp(sigma_e[S_id[n]]));
      beta_n[n] = beta[S_id[n]];
      lapse_n[n] = inv_logit(lapse[S_id[n]]);
  }
  
  
  // Pre-compute subject-level values indexed by trial

  // Vectorize mu calculation
  vector[N] mu = XD - (inv_logit(beta_n) - 0.5) * 2;
  
  // Vectorize theta calculation
  vector[N] mu_std = mu ./ sigma1_sq_n;
  
  vector[N] theta = (lapse_n / 2) + (1 - 2 * lapse_n / 2) .* Phi(mu_std);
  
  // Compute numerator for action = 1 trials (a==1) and action = 0 trials (a==0)
  vector[N] numerator = ((a_vec .* (mu + (sigma_e_n ./ sigma1_sq_n) .* exp(std_normal_lpdf( mu_std)) ./ Phi(mu_std)) + (1 - a_vec) .* (mu - (sigma_e_n ./ sigma1_sq_n) .* exp(std_normal_lpdf(- mu_std)) ./ Phi(- mu_std))) * Cc .* X) ./ sigma2_n;
  
  // Calculate theta_conf with vectorized Phi
  vector[N] theta_conf = a_vec .* (Phi(numerator ./ sqrt(1 + ((square(Cc)*square(X)) ./ sigma2_n)))) + (1 - a_vec) .* (1 - (Phi(numerator ./ sqrt(1 + ((square(Cc)*square(X)) ./ sigma2_n)))));
}
...
model {
  ...
  profile("likelihood") {
  for (n in 1:N) {
    target += ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + meta_bias[S_id[n]] + meta_bias_beta[S_id[n]] * interval[n], exp(confprec[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);
  }
...
generated quantities {
  ...
  // Fully vectorized log-likelihood calculations
  for(n in 1:N){
  ...
  log_lik = log_lik_bin + log_lik_conf;
