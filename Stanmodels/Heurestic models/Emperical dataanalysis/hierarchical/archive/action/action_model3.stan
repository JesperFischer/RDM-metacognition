...
transformed parameters{
  ...
  if(a[n] == 1){
    theta_conf[n] = inv_logit(metauninc_p[S_id[n]]) + (1-inv_logit(metauninc_p[S_id[n]])) * Phi(exp(beta_p[S_id[n]] - exp(metauncor_p[S_id[n]]) +  meta_un_beta[S_id[n]] * interval[n]) * (XD[n] - (inv_logit(alpha_p[S_id[n]])-0.5)*2));
...
model {
  ...
  gm[5] ~ normal(0,1); //global mean of meta uncertainty

  gm[6] ~ normal(-1,1); //global mean of lowerasympotete
