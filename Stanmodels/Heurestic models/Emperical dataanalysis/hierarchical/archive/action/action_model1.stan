...
transformed parameters{
  ...
  if(a[n] == 1){
    theta_conf[n] = inv_logit(metauninc_p[S_id[n]]) / 2 + (1-inv_logit(metauninc_p[S_id[n]]) / 2) * Phi(exp(beta_p[S_id[n]] - exp(metauncor_p[S_id[n]])) * (XD[n] - (inv_logit(alpha_p[S_id[n]])-0.5)*2));
