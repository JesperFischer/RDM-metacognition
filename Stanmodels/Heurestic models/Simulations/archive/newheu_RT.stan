...
transformed parameters{
  ...
  real confprec_p = gm[4];
  real metauncor_p = gm[5];
  real metabias_p = gm[6];
  ...
  real sigma2 = exp(beta_p)^2 + exp(metauncor_p)^2;  // Now squaring to get variances
  ...
  for (n in 1:N) {
  theta[n] = Phi( (XD[n]-(inv_logit(alpha_p)-0.5)*2) / sqrt(exp(beta_p)^2 + exp(beta2_p)^2));
  ent[n] = -theta[n] * log(theta[n]) - (1-theta[n]) * log(1-theta[n]);
  if(a[n] == 1){
    theta_conf[n] = Phi( (1/(sqrt(1 + (Cc^2*X[n]^2) / sigma2))) * ((XD[n] - (inv_logit(alpha_p)-0.5)*2) * X[n] * Cc) / sigma2 );
    
  }else if(a[n] == 0){
    theta_conf[n] =  1-Phi( (1/(sqrt(1 + (Cc^2*X[n]^2) / sigma2))) * ((XD[n] - (inv_logit(alpha_p)-0.5)*2) * X[n] * Cc) / sigma2 );
  }
...
model {
  ...
  // gm[4] ~ normal(-4,2); //global mean of lapse

  gm[4] ~ normal(2,2); //global mean of confprec

  gm[5] ~ normal(-1,1); //global mean of meta uncertainty

  gm[6] ~ normal(0,1); //global mean of metabias

  gm[7] ~ normal(-1,1); //global mean of metabias
  gm[8] ~ normal(0,2); //global mean of metabias
  gm[9] ~ normal(-1,1); //global mean of metabias
  ...
  for (n in 1:N) {
    ...
    target += ord_beta_reg_lpdf(C[n] | logit(theta_conf[n])+ metabias_p, exp(confprec_p), c0, c11);   // likelihood for confidence on correct trials

    target += lognormal_lpdf(rts[n] | rtint_p + rtbeta_p * ent[n], exp(rtsd_p));   // likelihood for confidence on correct trials
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + metabias_p, exp(confprec_p), c0, c11);   // likelihood for confidence on correct trials
