...
transformed parameters{
  ...
  for (n in 1:N) {
  theta[n] = inv_logit(lapse_p)/2 + (1-2*inv_logit(lapse_p)/2) * Phi(sqrt(exp(beta_p) + exp(beta2_p)) * (XD[n]-(inv_logit(alpha_p)-0.5)*2));

  if(a[n] == 1){
    theta_conf[n] = Phi( (1/(sqrt(1 + (exp(beta_p) + exp(metauncor_p)) * (Cc^2*X[n]^2)))) * ((exp(beta_p) + exp(metauncor_p))) * ((XD[n] - (inv_logit(alpha_p)-0.5)*2) * X[n] * Cc));
    
  }else if(a[n] == 0){
    theta_conf[n] =  1-Phi( (1/(sqrt(1 + (exp(beta_p) + exp(metauncor_p)) * (Cc^2*X[n]^2)))) * ((exp(beta_p) + exp(metauncor_p))) * ((XD[n] - (inv_logit(alpha_p)-0.5)*2) * X[n] * Cc));
  }
...
model {
  ...
  gm[3] ~ normal(2,2); //global mean of confprec

  gm[4] ~ normal(-4,2); //global mean of lapse

  gm[5] ~ normal(2,2); //global mean of confprec

  gm[6] ~ normal(0,2); //global mean of meta uncertainty

  gm[7] ~ normal(0,2); //global mean of metabias
