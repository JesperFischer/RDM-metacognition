...
transformed parameters{
  ...
  real metauninc_p = gm[6];
  real metabias_p = gm[7];
  real  meta_un_beta = gm[8]; 
  real  meta_bias_beta = gm[9];
  ...
  for (n in 1:N) {
  theta[n] = psycho_ACC(XD[n], exp(beta_p), (inv_logit(alpha_p)-0.5)*2, inv_logit(lapse_p) / 2) ;

  if(a[n] == 1){
    theta_conf[n] = inv_logit(metauninc_p) + (1-inv_logit(metauninc_p)) * Phi(exp(beta_p - exp(metauncor_p) +  meta_un_beta * interval[n]) * (XD[n] - (inv_logit(alpha_p)-0.5)*2));
...
model {
  ...
  for (n in 1:N) {
    ...
    target += ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + metabias_p+  meta_bias_beta * interval[n], exp(confprec_p), c0, c11);   // likelihood for confidence on correct trials
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + metabias_p+  meta_bias_beta * interval[n], exp(confprec_p), c0, c11);   // likelihood for confidence on correct trials
