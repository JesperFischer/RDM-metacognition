...
In this section, we first describe the general structure of static models of perceptual decision-making and confidence ratings. We then derive and simulate from our proposed model, demonstrating that it can produce the key patterns highlighted in the [Design considerations](part1-models-data.qmd).
...
This can be shown to yield (see the full derivation in the collapsible block below):
...
We now assume that the agent computes the posterior probability of D given the evidence he collected. This means that for the case where D = 1 we have from Bayes' theorem that:
...
$$
writing out the squares $(e + X_i)^2 = e^2 + X_i^2 + 2\cdot e \cdot X_i$ & $(e - X_i)^2 = e^2 + X_i^2 - 2\cdot e \cdot X_i$, we end up with:
...
For a full derivation, see the [Truncated Normal derivation](Additional Derivations/Truncated Normal.Rmd).
