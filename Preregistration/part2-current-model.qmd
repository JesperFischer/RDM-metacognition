...
In this section, we first describe the general structure of static models of perceptual decision-making and confidence ratings. We then derive and simulate from our proposed model, demonstrating that it can produce the key patterns highlighted in the [Design considerations](Models and Data in Perceptual Decision-Making with Confidence.Rmd).
...
::: {.callout-note collapse="true" title="Derivation of the probability of correctness given noisy evidence"}
...
cache_file <- here("Preregistration", "cached-results", "platenotation.png")
if (file.exists(cache_file)) {
  magick::image_read(cache_file)
...
First, we examine the distribution of internal evidence over the objective signed stimulus (i.e., the product of stimulus intensity and true state) across the two parameters governing choices: $\sigma_e$ (evidence encoding noise) and $\sigma_c$ (choice variability).

```{r}
#| label: plot-evidence-accuracy
#| fig-height: 6
#| fig-width: 10
#| warning: false
#| message: false
...
From the above plot we see that $\sigma_e$ broadens the range of observed evidence (making it noisier for the same external stimulus); moving down the rows, the y-axis range increases substantially. When $\sigma_c$ is low (left column), choices are consistent: positive evidence paired with a positive signed stimulus (i.e., $D = 1$) yields a correct response. As $\sigma_c$ increases, this relationship becomes noisier, and correct and incorrect choices become less distinguishable across the zero-evidence and zero-signed-stimulus lines.

Another way to display this is by plotting actions instead of accuracy:

```{r}
#| label: plot-evidence-action
#| fig-height: 6
#| fig-width: 10
#| warning: false
#| message: false
...
cache_file <- here("Preregistration", "cached-results", "df_interaction_ordbeta.rds")
if (file.exists(cache_file)) {
  df_interaction <- readRDS(cache_file)
} else if (!nzchar(Sys.getenv("CI"))) {
  set.seed(199742)
  ...
  stop("Cached results not found. Run build-cache.R locally first.")
}
```

First, we fix $\sigma_c = 1$ and examine how $\sigma_e$ and $\sigma_m$ change confidence ratings as a function of signed stimulus intensity:

```{r}
#| label: plot-conf-sigma-e-m
#| fig-height: 6
#| fig-width: 10
#| message: false
#| warning: false
...
We want to marginalize over $r_i$, i.e., calculate the expectation of the above expression across $r_i$. This can be done using the general formula for the expectation of a continuous random variable:
...
We write the expectation of $r_i$ rather than the distributional mean ($X_iD_i - \beta$), because this expectation will be conditional on the choice (recall that we are considering $a_i = 1$).
...
It can be shown that this conditional expectation equals the following (for a full derivation, see below):


::: {.callout-note collapse="true" title="Calculating conditional expectations"}
...
The final equations therefore become:
...
The terms inside the cumulative normal distribution function have clear interpretations.
...
```{r}
#| label: source-simulation-fns
#| fig-height: 6
#| fig-width: 10
#| warning: false
#| message: false

# Define breaks for binning (used by all subsequent plots)
breaks <- seq(-1, 1, by = 0.1)

cache_file_grid <- here("Preregistration", "cached-results", "df_grid_components.rds")
cache_file_A <- here("Preregistration", "cached-results", "df_A_components.rds")
cache_file_B <- here("Preregistration", "cached-results", "df_B_components.rds")

if (all(file.exists(cache_file_grid, cache_file_A, cache_file_B))) {
  df_grid <- readRDS(cache_file_grid)
  df_A <- readRDS(cache_file_A)
  df_B <- readRDS(cache_file_B)
} else if (!nzchar(Sys.getenv("CI"))) {
  ...
  # --- df_A: what sigma_m does — fix sigma_e & sigma_c, vary sigma_m as colour ---
  ...
  # --- df_B: shape of S_i — fix sigma_m, vary sigma_e (rows) x sigma_c (cols) ---
  ...
  saveRDS(df_grid, cache_file_grid)
  saveRDS(df_A, cache_file_A)
  saveRDS(df_B, cache_file_B)
} else {
  stop("Cached results not found. Run build-cache.R locally first.")
}
...
* $\gamma$ is the lapse rate on binary choices, a stimulus-independent factor that shifts the asymptote of the psychometric function away from certainty, i.e., the asymptotes become $\gamma$ and $1-\gamma$.
...
* $c_0, c_1$ are the two cutpoints for the ordered beta likelihood. These parameters generalize the beta distribution to outcomes that include 0 and 1 ratings.
...
cache_file <- here("Preregistration", "cached-results", "validation_sims_1.rds")
if (file.exists(cache_file)) {
  cached <- readRDS(cache_file)
  df_mech <- cached$df_mech
  df_heu <- cached$df_heu
...
cache_file <- here("Preregistration", "cached-results", "validation_sims_2.rds")
if (file.exists(cache_file)) {
  cached <- readRDS(cache_file)
  df_mech <- cached$df_mech
  df_heu <- cached$df_heu
...
cache_file <- here("Preregistration", "cached-results", "loocomparison.rds")
results_file <- here("Preregistration", "Results", "loocomparison.rds")

if (file.exists(cache_file)) {
  loocomparison <- readRDS(cache_file)
} else if (file.exists(results_file)) {
  loocomparison <- readRDS(results_file)
  saveRDS(loocomparison, cache_file)
} else {
  warning("loocomparison.rds not found — using placeholder values")
  loocomparison <- matrix(c(0, 0, -0.5, 1.2), nrow = 2, ncol = 2)
}
...
```{r}
#| label: plot-posterior-predictive
#| fig-height: 12
#| fig-width: 18
#| message: false
#| error: false
...
cache_prior <- here("Preregistration", "cached-results", "prior_predictive.rds")
cache_effects <- here("Preregistration", "cached-results", "effect_sims.rds")

if (all(file.exists(cache_prior, cache_effects))) {
  df_all <- readRDS(cache_prior)
  df_effects <- readRDS(cache_effects)
} else if (!nzchar(Sys.getenv("CI"))) {
...
```{r}
#| label: sequential-sampling-plot
#| fig-height: 8
#| fig-width: 12
#| warning: false
#| error: false
