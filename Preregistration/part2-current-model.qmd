...
As can be seen from equation (2), computing the probability of being correct requires access to the stimulus intensity $X_i$. Although this is assumed in some models [@FLEMINGNeuralMediatorsChangesMindPerceptualDecisions2018a], we believe the rater does not have access to the objective stimulus intensity. We return to this issue later, where note that we fix this quantity to one for the current registration. Interested readers are referred to @RAUSCHFoldedXpatternNotNecessarilyStatisticalSignature2019, where simulations are conducted with different generative processes for the stimulus intensity.
...
#     mutate(bin    = cut(XD, breaks = breaks, include.lowest = TRUE, labels = FALSE),
#            XD_bin = (breaks[bin] + breaks[bin + 1]) / 2,
#            action_label  = ifelse(a == 1, "up", "down")) %>%
#     group_by(XD_bin, action_label) %>%
#     summarise(mean_S = mean(S_i), .groups = "drop") %>%
#     ggplot(aes(x = XD_bin, y = mean_S,
#                color = action_label)) +
#     annotate("rect", xmin = 0,    xmax =  Inf, ymin = 0,    ymax =  Inf, fill = "green", alpha = .15) +
...
#     labs(x = "Signed Stimulus (XD)", y = expression(S[i]),
#          colour = NULL, linetype = NULL) +
...
# ggsave(here::here("tester_up.tiff"),pA,height = 16,width = 43, units = "cm", dpi = 300)
