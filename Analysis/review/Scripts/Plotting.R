...
get_marginal_estimates = function(fit){
  ...
  if(length(unique(colnames(df))) == 12){
    df = df %>% rename_with(~c(gm_names_full[1:8],gm_names_full[10],"rho_p_rt","rho_p_conf","rho_rt_conf"))
  }else if(length(unique(colnames(df))) == 13){
    df = df %>% rename_with(~c(gm_names_full[1:9],gm_names_full[10],"rho_p_rt","rho_p_conf","rho_rt_conf"))
  }else if(length(unique(colnames(df))) == 14){
    df = df %>% rename_with(~c(gm_names_full[1:11],"rho_p_rt","rho_p_conf","rho_rt_conf"))
...
Plot_group_predictive_psycho = function(predictions, df, ACC = F, n_bins = NULL) {
    ...
    geom_pointrange(data = dataq %>%
                      mutate(name = ifelse(name == "RT","Response time",
                             ...
                             name = factor(name, levels = c("Binary choice",
                                                            "Response time",
                                                            "Confidence"))),
                    aes(x = X, y = mean, ymin = q5, ymax = q95, fill = Correct),
                    shape = 21, color = "black", alpha = 0.5) +
    geom_line(aes(x = X, y = mean, color = Correct), linewidth = 1) +
    facet_wrap(~name, scales = "free_y", ncol = 3) +
    scale_y_continuous(breaks = scales::pretty_breaks(n = 4))+
  ...
  return(list(plot_mean,plot_residuals,combined_plot))
  
}


Plot_group_predictive_psycho_nbin = function(predictions, df, ACC = F, n_bins = NULL) {
  ...
  dataq = df %>% pivot_longer(cols = c("RT","Confidence","Y"), names_to = "name", values_to = "value") %>% 
    mutate(Correct = ifelse(name == "RT",NA,ifelse(name == "Y",NA,Correct))) %>%
    mutate(Correct = ifelse(Correct == 1, "Correct", "Incorrect"))
  ...
  if(ACC){
    bin_resid = df_with_pred %>%
      mutate(residual_mean = ACC - pred_prob_faster,
             name = "Type-1")
  }else if(!ACC){
    bin_resid = df_with_pred %>%
      mutate(residual_mean = Y - pred_prob_faster,
             name = "Type-1")
  }
  ...
  # Calculate residuals at trial level, then aggregate
  residuals_data = bind_rows(
    bin_resid%>% select(residual_mean,name,Correct,X),
    
    df_with_pred %>%
      mutate(residual_mean = RT - pred_rt,
             name = "RT") %>% select(residual_mean,name,Correct,X), 
    
    
    df_with_pred %>%
      mutate(residual_mean = Confidence - pred_conf,
             name = "Confidence") %>% select(residual_mean,name,Correct,X)
  ) %>% 
    mutate(Correct = ifelse(name == "RT",NA,ifelse(name == "Type-1",NA,Correct))) %>%
    mutate(Correct = ifelse(Correct == 1, "Correct", "Incorrect"))
  ...
  plot_mean = predictionsq_mean %>%
    ...
    geom_point(data = dataq %>%
                      mutate(name = ifelse(name == "RT","Response time",
                                           ifelse(name == "Y","Binary choice","Confidence")),
                             name = factor(name, levels = c("Binary choice",
                                                            "Response time",
                                                            "Confidence"))),
                    aes(x = X, y = value, fill = Correct),
                    shape = 21, color = "black", alpha = 0.5, size = 1.5) +
    geom_line(aes(x = X, y = mean, color = Correct), linewidth = 1) +
    facet_wrap(~name, scales = "free_y", ncol = 3) +
    scale_y_continuous(breaks = scales::pretty_breaks(n = 4))+
    scale_color_manual(values = c("darkgreen","darkred","grey"))+
