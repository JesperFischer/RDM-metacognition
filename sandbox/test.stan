...
transformed parameters{
  ...
  vector[S] alpha_p = param[,1];
  
  vector[S] beta_p = param[,2];
  
  vector[S] lapse_p = param[,3];
  
  vector[S] confprec_p = param[,4];

  vector[S] metauncor_p = param[,5];

  vector[S] metauninc_p = param[,6];
  
  vector[S] metabias_p = param[,7];
  ...
  for (n in 1:N) {
  theta[n] = psycho_ACC(XD[n], exp(beta_p[S_id[n]]), alpha_p[S_id[n]], inv_logit(lapse_p[S_id[n]]) / 2) ;

  if(a[n] == 1){
    theta_conf[n] = inv_logit(metauninc_p[S_id[n]]) / 2 + (1-inv_logit(metauninc_p[S_id[n]]) / 2) * Phi(exp(beta_p[S_id[n]] - exp(metauncor_p[S_id[n]]) +  meta_un_beta[S_id[n]] * interval[n]) * (XD[n] - alpha_p[S_id[n]]));
...
model {
  ...
  gm[1] ~ normal(0,5); //global mean of threshold 

  gm[2] ~ normal(2,2); //global mean of beta

  gm[3] ~ normal(-4,2); //global mean of lapse

  gm[4] ~ normal(2,2); //global mean of confprec

  gm[5] ~ normal(0,2); //global mean of meta uncertainty

  gm[6] ~ normal(-2,2); //global mean of lowerasympotete

  gm[7] ~ normal(0,1); //global mean of metabias
  gm[8] ~ normal(0,0.5); 
  gm[9] ~ normal(0,0.5); 
  ...
  for (n in 1:N) {
    ...
    target += ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + metabias_p[S_id[n]]+  meta_bias_beta[S_id[n]] * interval[n], exp(confprec_p[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);   // likelihood for confidence on correct trials
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + metabias_p[S_id[n]]+  meta_bias_beta[S_id[n]] * interval[n], exp(confprec_p[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);   // likelihood for confidence on correct trials
