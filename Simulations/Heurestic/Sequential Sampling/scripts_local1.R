...
sim_new_subjects = function(max_subjects = 100){
  ...
  purrr::map_dfr(
    ...
    pivot_longer(cols = c("bf8")) %>% 
    # filter(value < 30) %>% 
    ggplot(aes(x = n_subs, y =log(value), group = draw_id, col = prior_sd))+
    geom_line()+
    geom_point()+
    facet_wrap(~draw_id)+
  ...
  purrr::map_dfr(
    ...
    ggplot(aes(x = n_subs, y = mean,ymin = q5,ymax = q95, group = draw_id))+
    geom_pointrange(position = position_dodge(width = 0.2))+
    geom_hline(aes(yintercept = simulated), col = "red",linetype = 2)+
    facet_grid(~draw_id, scales = "free")+
    theme_classic()
  ...
  files = list.files((here::here("Simulations","Heurestic","Sequential Sampling","results","pure_samples")), full.names = T)
  ...
  simulated_means = purrr::map_dfr(
    ...
    select(variable,simulated,draw_id,prior_sd) %>% 
    # select(ess_bulk, ess_tail, rhat)
    distinct()
  ...
  purrr::map_dfr(
    ...
    # mutate(draw_id = if_else(row_number() <= 6, draw_id + 1, draw_id)) %>% 
    pivot_longer(cols = c("bf8")) %>% 
    inner_join(.,simulated_means) %>% 
    # filter(value < 30) %>% 
    ggplot(aes(x = n_subs, y =log(value), group = draw_id))+
    geom_line()+
    geom_point()+
  ...
  purrr::map_dfr(
    ...
    filter(div == 0 & tree == 0, abs(rhat) < 1.01) %>% 
    mutate(prior_sd = as.factor(prior_sd)) %>% 
    ggplot(aes(x = n_subs, y = mean,ymin = q5,ymax = q95, group = draw_id))+
    geom_pointrange(position = position_dodge(width = 0.2))+
    geom_hline(aes(yintercept = simulated), col = "red",linetype = 2)+
