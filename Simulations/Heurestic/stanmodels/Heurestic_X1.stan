...
generated quantities {
  ...
  // real dp = 2 * uniqueX / sqrt(exp(sigma_e)^2 + exp(sigma_k)^2);
  // real mdp = 1 / sqrt(exp(sigma_k)^2 + exp(sigma_e)^2 + exp(sigma_m)^2);    

  real mratio = sqrt(exp(sigma_e)^2 + exp(sigma_k)^2)/sqrt(exp(sigma_e)^2 + exp(sigma_m)^2);    
