...
functions {
  ...
  real metad__2__normal__absolute__multinomial_lpmf(array[] int Y, real M, real dprime, real c, real z_meta_c2_0_1, real z_meta_c2_1_1) {
  int K = 2; // number of confidence levels

  real meta_dprime = M * dprime;
  real meta_c = c;
  vector[K-1] meta_c2_0 = meta_c - cumulative_sum([z_meta_c2_0_1]');
  vector[K-1] meta_c2_1 = meta_c + cumulative_sum([z_meta_c2_1_1]');
...
model {
  ...
  if (!prior_only) {
    ...
    mu = exp(mu);
    for (n in 1:N) {
      target += metad__2__normal__absolute__multinomial_lpmf(Y[n] | mu[n], dprime, c, metac2zero1diff, metac2one1diff);
    }
  }
