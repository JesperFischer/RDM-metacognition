...
transformed parameters{
  ...
  vector[S]  omega = param[,10];  // meta uncertainty on incorrect trials
  ...
  if(a[n] == 1){
    theta_conf[n] = Phi( (((mu + omega[S_id[n]] * exp(sigma_e[S_id[n]])^2/sigma1[S_id[n]] * lambda(mu/sigma1[S_id[n]])) * Cc*X[n]) / sigma2) * 1/(sqrt(1 + ((Cc^2*X[n]^2) / sigma2))));
    
  }else if(a[n] == 0){
    theta_conf[n] =  1-Phi( (((mu - omega[S_id[n]] * exp(sigma_e[S_id[n]])^2/sigma1[S_id[n]] * lambda(-mu/sigma1[S_id[n]])) * Cc*X[n]) / sigma2) * 1/(sqrt(1 + ((Cc^2*X[n]^2) / sigma2))));
  }
