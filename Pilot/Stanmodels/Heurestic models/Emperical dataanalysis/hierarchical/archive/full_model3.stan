...
transformed parameters{
  ...
  vector[S]  meta_un_beta = param[,7];  // meta uncertainty on incorrect trials
...
model {
  ...
  for (n in 1:N) {
    ...
    target += ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + meta_bias[S_id[n]] + meta_bias_beta[S_id[n]] * interval[n], exp(conf_prec1[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);   // likelihood for confidence on correct trials 
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + meta_bias[S_id[n]] + meta_bias_beta[S_id[n]] * interval[n], exp(conf_prec1[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);
