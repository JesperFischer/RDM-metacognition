...
transformed parameters{
  ...
  if(a[n] == 1){
    theta_conf[n] = inv_logit(metauninc_p[S_id[n]]) / 2 + (1-inv_logit(metauninc_p[S_id[n]]) / 2) * Phi(exp(beta_p[S_id[n]] - exp(metauncor_p[S_id[n]]) +  meta_un_beta[S_id[n]] * interval[n]) * (XD[n] - (inv_logit(alpha_p[S_id[n]])-0.5)*2));
...
model {
  ...
  gm[6] ~ normal(0,1); //global mean of lowerasympotete
  ...
  for (n in 1:N) {
    ...
    target += ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + metabias_p[S_id[n]], exp(confprec_p[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);   // likelihood for confidence on correct trials
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + metabias_p[S_id[n]], exp(confprec_p[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);   // likelihood for confidence on correct trials
