...
transformed parameters{
  ...
  if(a[n] == 1){
    theta_conf[n] = Phi( (((mu + exp(sigma_e)^2/sqrt(sigma1) * lambda(mu/sqrt(sigma1))) * Cc*X[n]) / sigma2) * 1/(sqrt(1 + ((Cc^2*X[n]^2) / sigma2))));
    
  }else if(a[n] == 0){
    theta_conf[n] =  1-Phi( (((mu - exp(sigma_e)^2/sqrt(sigma1) * lambda(-mu/sqrt(sigma1))) * Cc*X[n]) / sigma2) * 1/(sqrt(1 + ((Cc^2*X[n]^2) / sigma2))));
  }
