...
transformed parameters{
  ...
  real  sigma_k = gm[3];
  real  sigma_m = gm[4];
  real  meta_bias = gm[5];  // meta uncertainty on incorrect trials
  real  lapse = gm[6];  // meta uncertainty on incorrect trials
  
  real  confprec = gm[7];  // meta uncertainty on incorrect trials
  real  omega = gm[8];  // meta uncertainty on incorrect trials
  ...
  if(a[n] == 1){
    theta_conf[n] = Phi( (((mu + (omega) * exp(sigma_e)^2/sqrt(sigma1) * lambda(mu/sqrt(sigma1))) * Cc*X[n]) / sigma2) * 1/(sqrt(1 + ((Cc^2*X[n]^2) / sigma2))));
    
  }else if(a[n] == 0){
    theta_conf[n] =  1-Phi( (((mu - (omega) *exp(sigma_e)^2/sqrt(sigma1) * lambda(-mu/sqrt(sigma1))) * Cc*X[n]) / sigma2) * 1/(sqrt(1 + ((Cc^2*X[n]^2) / sigma2))));
  }
...
model {
  ...
  gm[8] ~ normal(1,1); //global mean of meta bias
