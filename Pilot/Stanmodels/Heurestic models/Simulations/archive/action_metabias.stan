...
transformed parameters{
  ...
  if(a[n] == 1){
    theta_conf[n] = inv_logit(metauninc)+ (1-inv_logit(metauninc)) * Phi(exp(beta - exp(metauncor)) * (X[n] - (inv_logit(alpha)-0.5)*2));
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]) + metabias, exp(confprec), c0, c11);   // likelihood for confidence on correct trials
