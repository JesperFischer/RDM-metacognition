...
model {
  ...
  for (n in 1:N) {
    target += ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]), exp(confprec[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);   // likelihood for confidence on correct trials 
  }
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(C[n] | logit(theta_conf[n]), exp(confprec[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);
