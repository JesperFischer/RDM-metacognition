...
transformed parameters{
  ...
  theta[n] =  (inv_logit(lapse)/2) + (1-2*inv_logit(lapse)/2) * Phi( mu / sqrt(sigma1));
  
  rt_mu[n] = rt_int + rt_beta * (-theta[n] * log(theta[n]) - (1- theta[n] * log(1-theta[n])));


  if(a[n] == 1){
    theta_conf[n] = Phi( (((mu + exp(sigma_e)^2/sqrt(sigma1) * lambda(mu/sqrt(sigma1))) * Cc*1) / sigma2) * 1/(sqrt(1 + ((Cc^2*1^2) / sigma2))));
    
  }else if(a[n] == 0){
    theta_conf[n] =  1-Phi( (((mu - exp(sigma_e)^2/sqrt(sigma1) * lambda(-mu/sqrt(sigma1))) * Cc*1) / sigma2) * 1/(sqrt(1 + ((Cc^2*1^2) / sigma2))));
  }
...
model {
  ...
  gm[7] ~ normal(0,1); //global mean of meta uncertainty
  gm[8] ~ normal(0,0.5); //global mean of meta bias
  gm[9] ~ normal(-1,1); 
  gm[10] ~ normal(0.2,0.3); 
  gm[11] ~ normal(-4,2); 
  ...
  for (n in 1:N) {
    ...
    target += lognormal_lpdf(RT[n] - inv_logit(rt_ndt) * minRT | rt_mu[n], exp(rt_sd));   // likelihood for the outcomes
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_rt[n] = lognormal_lpdf(RT[n] | rt_mu[n], exp(rt_sd));   // likelihood for confidence on correct trials

    log_lik[n] = log_lik_bin[n]+log_lik_conf[n] + log_lik_rt[n];
  }
