...
transformed parameters{
  ...
  vector[S]  meta_bias = (param[,7]);  // meta uncertainty on correct trials
...
model {
  ...
  gm[7] ~ normal(0,0.5); //global mean of meta bias
  ...
  for (n in 1:N) {
    target += ord_beta_reg_lpdf(C[n] | logit(theta_conf[n])+ meta_bias[S_id[n]], exp(confprec[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);   // likelihood for confidence on correct trials 
  }
...
generated quantities {
  ...
  for(n in 1:N){
    ...
    log_lik_conf[n] = ord_beta_reg_lpdf(C[n] | logit(theta_conf[n])+ meta_bias[S_id[n]], exp(confprec[S_id[n]]), c0[S_id[n]], c11[S_id[n]]);
