...
transformed parameters{
  ...
  sigma2[n] = exp(sigma_e[S_id[n]])^2 + exp(sigma_m[S_id[n]]  +  sigmam_beta[S_id[n]] * interval[n])^2;
  
  mu[n] = (XD[n] - (inv_logit(beta[S_id[n]])-0.5)*2);

  theta[n] = Phi( mu[n] / sigma1[S_id[n]]);

  if(a[n] == 1){
    K_i[n] = 1/(sqrt(1 + ((Cc^2) / sigma2[n])));
    S_i[n] = ((exp(sigma_e[S_id[n]])^2/sigma1[S_id[n]]) * lambda(mu[n]/sigma1[S_id[n]]) * Cc) / sigma2[n];
    I_i[n] = mu[n] * Cc / sigma2[n];
    
    theta_conf[n] =  Phi((I_i[n] + S_i[n]) * K_i[n]);
