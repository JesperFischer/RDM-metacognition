...
transformed parameters{
  ...
  vector[S]  sigma_e = (param[,1]);
  vector[S]  sigma_k = param[,2];  // confidence precision
  vector[S]  sigma_m = (param[,3]);  // meta uncertainty on correct trials
  vector[S]  meta_bias = param[,4];  // meta uncertainty on incorrect trials
  // vector[S]  lapse = param[,5];  // meta uncertainty on incorrect trials
  
  vector[S]  confprec = param[,5];  // meta uncertainty on incorrect trials
  vector[S]  sigmam_beta = param[,6];  // meta uncertainty on incorrect trials
  vector[S]  meta_bias_beta = param[,6];  // meta uncertainty on incorrect trials
...
model {
  ...
  gm[1] ~ normal(-2,2); //global mean of sigma_e
  gm[2] ~ normal(-2,2); //global mean of sigma_k
  gm[3] ~ normal(-2,2); //global mean of sigma_m
  gm[4] ~ normal(0,0.5); //global mean of meta bias
  // gm[5] ~ normal(-4,2); //global mean of lapse rate
  
  gm[5] ~ normal(2,2); //global mean confidence precision
  gm[6] ~ normal(0,0.5); //global mean of difference in sigma_m from PDW
  gm[7] ~ normal(0,0.5); //global mean of difference in meta_bias from PDW
  
  to_vector(z_expo) ~ std_normal();

  // same as above (indicies) but between subject variances

  tau_u[1:5] ~ normal(1 , 1);
  tau_u[6] ~ normal(0 , 0.5);
  tau_u[7] ~ normal(0 , 0.5);
  
  beta ~ normal(gm_beta,tau_beta);
  
  gm_beta ~ normal(0,0.5);
  tau_beta ~ normal(1,1);
